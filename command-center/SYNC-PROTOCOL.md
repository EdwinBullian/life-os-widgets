...
> across **Cowork**, the **laptop daemon**, **Windows Task Scheduler**, and **non-Cowork (DeepSeek) jobs**.
...
scheduler, the laptop daemon's supervisor, Windows Task Scheduler, DeepSeek/gateway rows), no single tool sees them
...
- For DeepSeek/non-Cowork agents, the same rule goes in their system prompt / handoff doc.
...
| DeepSeek / gateway | the Agent Dispatch queue rows + gateway job config |
...
## Why this covers the Cowork + DeepSeek split specifically
The whole reason the Command Center exists is that you run agents in **two ecosystems** and need one view. Rule 1
keeps Cowork and DeepSeek sessions honest at write time; Rule 2's per-host readers mean even tasks that Cowork's
